...
Zed clones the grammar from `packages/tooling/tree-sitter-loam` via `file://`. That directory must be its own git repo (`make grammar` creates it). If install fails with **failed to compile grammar 'loam'**:
