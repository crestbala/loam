...
//
// Generates <appdir>/app_routes.yuga from the app's routes/ tree; builds
...
//
//     routes/page.yuga          page for the directory's URL   fn page(p: Params)
//     routes/layout.yuga        chrome, then content           fn layout(build: fn())
//     routes/not-found.yuga     404 page                        fn not_found()
//     routes/error.yuga          route error page               fn error(msg: string)
//     routes/loading.yuga        pending page                   fn loading()
//     routes/[slug]/page.yuga   /:slug param
//     routes/(group)/...        URL-invisible group
//
// Layouts compose outermost-first down the directory chain. The root layout
// (`routes/layout.yuga`) is applied once around the router, so its state
...
function generate(appdir: string): string {
  ...
  const out = path.join(appdir, "app_routes.yuga");
  ...
  for (const { parts, dir, files } of entries) {
    if (files.has("layout.yuga")) {
      const al = aliasFor(parts, "layout");
      layouts.set(key(parts), al);
      note(al, path.join(dir, "layout.yuga"));
    }
    if (files.has("page.yuga")) {
      const pal = aliasFor(parts, "page");
      note(pal, path.join(dir, "page.yuga"));
      const src = Deno.readTextFileSync(path.join(dir, "page.yuga"));
      builders.push({
        parts,
        alias: pal,
    ...
    if (files.has("not-found.yuga")) {
      const al = aliasFor(parts, "not_found");
      note(al, path.join(dir, "not-found.yuga"));
      if (parts.length === 0) rootNf = al;
    }
  }

  if (builders.length === 0) die(`zeus: no page.yuga under ${routesDir}`);
...
/** `<appdir>/app.yuga`, else `<appdir>/<dirname>.yuga`, else `main.yuga`. */
function findEntry(appdir: string): string {
  const base = path.basename(appdir);
  for (const name of ["app.yuga", base + ".yuga", "main.yuga"]) {
    const p = path.join(appdir, name);
    if (exists(p)) return p;
  }
  die(`zeus: no entry .yuga in ${appdir} (looked for app.yuga / ${base}.yuga / main.yuga)`);
}
...
function build(appdir: string, targets: TargetName[], base: string): number {
  ...
  const stem = path.basename(entry, ".yuga");
  let built = 0;
...
function collectMeta(appdir: string): MetaRec[] {
  ...
  const src = path.join(zdir, "meta_dump.yuga");
 ...
 * `import "pkg:name"` then resolves to `vendor/name/name.yuga`.
 */
...
function collectYuga(dir: string, out: string[]): void {
  ...
  for (const e of entries) {
    ...
    else if (e.isFile && p.endsWith(".yuga")) out.push(p);
  }
}
 ...
 *
 * Builds the web target, watches `.yuga` sources (the app plus the framework
...
function scaffoldFiles(name: string): Record<string, string> {
  ...
  return {
    ...
    // One entry for every host — not four copies of app.yuga.
    "app.yuga": sub(
      `//! __NAME__ — application entry. One entry, every host.\n` +
        `//!\n` +
        `//! \`zeus routes\` generates app_routes.yuga from routes/; this file\n` +
        `//! mounts it. \`zeus build\` emits all five targets from here.\n\n` +
        `import "std:zeus"\n` +
        `import "app_routes.yuga" as approutes\n` +
        `import "server/api.yuga" as api\n` +
        `import "tests/smoke.yuga" as smoketest\n\n` +
        `fn main() {\n` +
    ...
    "routes/page.yuga":
      `import "std:zeus"\n` +
      `import "std:router"\n` +
      `import "../components/card.yuga" as card\n\n` +
      `fn page(p: Params) {\n` +
    ...
    "routes/error.yuga":
      ...
      `}\n`,
    "routes/not-found.yuga":
      `import "std:zeus"\n\n` +
      `fn not_found() {\n` +
      `    zeus.Text("not found")\n` +
      `}\n`,

    "routes/blog/layout.yuga":
      `import "std:zeus"\n\n` +
      `fn layout(build: fn()) {\n` +
    ...
    "routes/blog/loader.yuga":
      ...
      `}\n`,
    "routes/blog/page.yuga":
      `import "std:zeus"\n` +
      `import "std:router"\n` +
      `import "loader.yuga" as blog_loader\n\n` +
      `fn page(p: Params) {\n` +
    ...
    "routes/blog/[slug]/page.yuga":
      `import "std:zeus"\n` +
      `import "std:router"\n` +
      `import "loader.yuga" as post_loader\n\n` +
      `fn page(p: Params) {\n` +
      ...
      `}\n`,
    "routes/blog/[slug]/error.yuga":
      `import "std:zeus"\n\n` +
      `fn error(msg: string) {\n` +
      `    zeus.Text("post not available")\n` +
      `}\n`,

    "routes/(marketing)/about/page.yuga":
      `import "std:zeus"\n` +
      `import "std:router"\n\n` +
      `fn page(p: Params) {\n` +
      `    zeus.Text("about")\n` +
      ...
      `}\n`,
    "routes/(marketing)/pricing/page.yuga":
      `import "std:zeus"\n` +
      `import "std:router"\n\n` +
      `fn page(p: Params) {\n` +
      `    zeus.Text("pricing")\n` +
    ...
    "tests/smoke.yuga":
      `//! \`#[test]\` fns. Import this from the entry and run \`yugac test app.yuga\`.\n\n` +
      `import "std:test"\n\n` +
...
function scaffold(appdir: string, name: string): number {
  ...
  // app.yuga imports app_routes.yuga, so generate it now: a fresh app must
