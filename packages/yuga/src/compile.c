...
/* Colon-separated framework roots searched for `std:` modules after the
   language std. Build-time default; a non-empty YUGA_PATH env variable is
...
/* Env YUGA_PATH (if set) followed by the build-time default, so a custom path
   adds roots without dropping zeus/http/maya. */
static const char *std_search_path(char *buf, size_t sz) {
    const char *env = getenv("YUGA_PATH");
    if (env && env[0] && strlen(YUGA_PATH) > 0) snprintf(buf, sz, "%s:%s", env, YUGA_PATH);
    else if (env && env[0]) snprintf(buf, sz, "%s", env);
    else snprintf(buf, sz, "%s", YUGA_PATH);
    return buf;
}

int yuga_std_dirs(const char **out, int max) {
    static char buf[1024];
    static char dirs[YUGA_MAX_STD_DIRS][1024];
    int n = 0;
    if (n < max) out[n++] = YUGA_STD_DIR;
...
static char *resolve_import(const char *importer, AstNode *im) {
    ...
    if (strncmp(spec, "pkg:", 4) == 0) {
        if (!pkg_module_lookup(spec + 4, path, sizeof path)) {
            yuga_error(im->loc, "cannot find package '%s' (run `zeus pkg sync`)", spec + 4);
            return NULL;
        }
        return normalize_path(path);
    }
...
static int load_imports(YugaModule *m) {
    ...
    for (size_t i = 0; i < p->as.program.import_count; i++) {
        ...
        if (!ip) return 1;
        im->as.import.resolved = yuga_dup(ip);
...
static int load_module(const char *path, const char *name) {
    ...
    if (name && strcmp(name, "http") != 0) {
        ...
        if (has_json && !has_std_json) {
            SourceLoc loc = ast->loc;
            AstNode *im = ast_import(yuga_dup("json"), yuga_dup("std:json"), loc);
            size_t ni = ast->as.program.import_count;
            AstNode **imps = (AstNode **)realloc(ast->as.program.imports, (ni + 1) * sizeof(AstNode *));
            if (!imps) yuga_fatal("out of memory");
            imps[ni] = im;
