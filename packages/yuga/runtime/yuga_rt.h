...
typedef struct YugaJmp {
    jmp_buf jb;
    struct YugaJmp *prev;
} YugaJmp;
#ifdef YUGA_RT_DEFINE_JMP
YugaJmp *yuga_jmp_top = NULL;
char yuga_jmp_msg[512];
void yuga_jmp_set_msg(const char *msg, int64_t len) {
    int64_t n = len;
    if (!msg) n = 0;
    if (n < 0) n = 0;
    if (n > (int64_t)sizeof(yuga_jmp_msg) - 1) n = (int64_t)sizeof(yuga_jmp_msg) - 1;
    if (n > 0) memcpy(yuga_jmp_msg, msg, (size_t)n);
    yuga_jmp_msg[n] = 0;
}
#else
extern YugaJmp *yuga_jmp_top;
extern char yuga_jmp_msg[512];
void yuga_jmp_set_msg(const char *msg, int64_t len);
#endif
#endif
...
#ifndef __wasm32__
    if (yuga_jmp_top) {
        yuga_jmp_set_msg(msg, (int64_t)strlen(msg));
        longjmp(yuga_jmp_top->jb, 1);
    }
#endif
    abort();
}
...
#ifndef __wasm32__
    if (yuga_jmp_top) {
        yuga_jmp_set_msg(msg.ptr ? msg.ptr : "", msg.len);
        longjmp(yuga_jmp_top->jb, 1);
    }
#endif
    abort();
