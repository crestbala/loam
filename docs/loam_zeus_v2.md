...
`next build` gives you a web app. `zeli build` gives you five targets from one
...
loam/
  ...
  zeus/                     the framework
    ...
    cli/                    zeli new / dev / build / fmt / test / inspect
...
in one route issue their requests concurrently; `zeli build` emits all five
...
```bash
# build then run
./bin/zeli build examples/zeus/myapp --targets macos
...
so if that's exported (e.g. left over from `make test`) the binary starts, does nothing, and exits 0. `unset ZEUS_HEADLESS LOAM_HEADLESS MAYA_HEADLESS` first. `zeli build` already strips them at build time, but the check is at *runtime*.
